...
int main() {  
  std::string jsonFilePath = "./calib16.json";  
  ...
  std::cout << "check rvec" << std::endl;
  //iterate rvec rotation vector and check if it is correct
  for (int i = 0; i < 3; ++i) {
    std::cout << cam_rvecs[i] << std::endl;
  }  
  cv::Mat R0, R1, R2;
  cv::Rodrigues(cam_rvecs[0], R0);
  cv::Rodrigues(cam_rvecs[1], R1);
  cv::Rodrigues(cam_rvecs[2], R2);  
  //std::cout << "cam_rvecs = " << cam_rvecs[1] << std::endl; 
  //std::cout << "cam_tvecs = " << cam_tvecs[1] << std::endl;
  std::cout << "Rotation Matrix of Camera 0= " << R0 << std::endl;
  std::cout << "Rotation Matrix of Camera 1= " << R1 << std::endl;
  std::cout << "Rotation Matrix of Camera 2= " << R2 << std::endl;
}
